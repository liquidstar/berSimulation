...
for index =1:length(K)
    ...
    for i=1:length(EbN0dB)
        ...
        y_ricean=h.*x+n; %received signal through Rician channel %Coherent Receiver for Rician Channel
        y_ricean_cap=y_ricean./h; %Assuming that h is known at the signal accurately
        r_ricean=real(y_ricean_cap)>0; %received symbols = 1 is real part > 0 or else it is 0 %Receiver for AWGN channel
        simBER_ricean(i)=sum(xor(d,r_ricean));
    end
    simBER_ricean=simBER_ricean/N; %Simulated BER; EbN0=10.^(EbN0dB/10); %Eb/N0 in Linear Scale semilogy(EbN0dB,simBER_ricean,plotStyle{index},'LineWidth',2);hold on legendInfo{index} = ['K = ' num2str(K(index))];
end
axis([-5 20 10^-5 10^0]);
%legend(legendInfo);
title('Eb/N0 Vs BER for BPSK over Rician Fading Channels with AWGN noise');
xlabel('Eb/N0(dB)'); ylabel('Bit Error Rate or Symbol Error Rate');