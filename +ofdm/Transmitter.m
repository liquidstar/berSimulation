	...
	methods
		function trans = Transmitter(rf, dataSource, ofdmVariant, Ts, fc)
			...
			if (trans.rfFlag)
				...
				trans.passBandAnalog = freqUpScale(ofdmVariant, baseBandAnalogI, baseBandAnalogQ, Ts, fc, t, Dt);
			end
		end	
	end
end
...
%% Map symbols to IFFT bins
 function bins = binBauds(baudMatrix, ofdmVariant)
	...
	bins = zeros(length(ofdmVariant),symbCount);
	j = 1;
...
%% Frequency upscaling for pass band 
 function bandPassSig = freqUpScale(ofdmVariant, baseBandAnalogI, baseBandAnalogQ, Ts, fc, t, Dt)
	% Mixing to get cos(fc+fm) + cos(fc-fm)
	n = length(ofdmVariant.subCarriers);
	fcPrime = fc - (n-1)/(2*Ts);
	mixedSig = baseBandAnalogI.*(cos(fcPrime*t)) + baseBandAnalogQ.*(sin(fcPrime*t));
	% High pass filter to get RF signal
	fs = Dt^-1;
	% Applying amplification
	bandPassSig = 1000*highpass(mixedSig, fcPrime, fs);
 end
